...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    result = list()
    for res in data:
        event = json.loads(res.contents[0])
        pycon = PyCon(name=event['name'],
...
def filter_pycons(pycons: List[PyCon],
    ...
    that continent.
    """
    return [pycon for pycon in pycons if get_continent(pycon.country) == continent and pycon.start_date.year == year ]

if __name__ == "__main__":
    pycon_events = get_pycon_events()
