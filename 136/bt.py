...
def check_bt(donor, recipient):
    ...
    """
    comp = _particular_antigen_comp(get_bloodtype(donor).value, get_bloodtype(recipient).value)
    return comp[0] >= 0 and comp[1] >= 0 and comp[2] >= 0
...
def get_bloodtype(donor):
    if isinstance(donor, Bloodtype):
        return donor

    if isinstance(donor, str):
        result = blood_type_text.get(donor, None)
    ...
    if isinstance(donor,int):
        ...
        if (donor_int >= 0) and (donor_int <= 7):
            return Bloodtype(donor_int)
        else:
            raise ValueError(f"Expected a donor int between 0 and 7 but got {donor_int}!")
