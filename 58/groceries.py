...
def create_parser():
    ...
    parser.add_argument('-a','--add', help="add item providing name (str), price"
            " (int), and craving (bool)", nargs=3)
    group.add_argument('-d', '--delete', help="delete a product by name (str)",
            nargs=1, type=str)
    group.add_argument('-l','--list', help="show itemsn in cart", action="store_true")
    parser.add_argument('-s','--search', help="search items by name", nargs=1)
...
def handle_args(args=None, cart=None):
    ...
    if args.add:
        ...
        if args.add[2].upper() == "TRUE":
            craving = True
        else:
            craving = False
        item = Item(product, price, craving)
        cart.add(item)
