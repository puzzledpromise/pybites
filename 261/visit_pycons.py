...
def update_pycons_lat_lon(pycons):
    ...
    with open(nominatim_responses, "r", encoding="utf-8") as f:
        data = json.load(f)
        for pycon in pycons:
            url = get_url(pycon.city, pycon.country)
            result = data[url][0]
            pycon.lat = float(result['lat'])
            pycon.lon = float(result['lon'])




def get_url(city, country):
    return f'https://nominatim.openstreetmap.org/search?q={city},{country}&format=json&accept-language=en'
...
def create_travel_plan(pycons):
    ...
    """
    sorted_pycons = sorted(pycons, key=lambda pycon : pycon.start_date)
    travels = zip(sorted_pycons[:-1], sorted_pycons[1:])
    return [Trip(a[0], a[1], _km_distance(a[0], a[1])) for a in travels]
...
if __name__ == "__main__":
    ...
    result = create_travel_plan(pycons)
    print(result)
